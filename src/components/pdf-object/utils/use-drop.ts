...
import { onScopeDispose, provide, ref, shallowRef } from 'vue-demi'
    ...
    if (target.value) {
      ...
      instance.value = Interact(dropTarget.value, { context: target.value }).dropzone({
        ...
        checker(dragEvent, event, dropped, dropzone, dropzoneElement, draggable, draggableElement) {
          ...
          return (
            dropped &&
            (target.value.contains(draggableElement) ||
              draggableElement.classList.contains('pdf-object--external'))
          )
        },
