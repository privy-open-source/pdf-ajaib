...
import { tryOnBeforeMount, tryOnBeforeUnmount, until } from '@vueuse/core'
import type { InjectionKey, Ref} from 'vue-demi';
import { computed, inject, reactive, unref, ref, watch } from 'vue-demi'
import type { PdfViewerContext } from '../pdf-viewers/main'
import { useSelector } from './utils/use-selector'
import { focus as focus_ } from './utils/focus'
...
export const PDF_OBJECT_CONTEXT: InjectionKey<ReturnType<typeof useObjectModel>> =
  Symbol('PDFObjects')
...
export function useSizeModel(props: PdfObjectSize) {
  ...
  watch(
    () => props.minWidth,
    (value) => {
      if (value && Number.isFinite(value)) minWidth.value = value
    }
  )
  ...
  watch(
    () => props.minHeight,
    (value) => {
      if (value && Number.isFinite(value)) minHeight.value = value
    }
  )
  ...
  watch(
    () => props.maxWidth,
    (value) => {
      if (value && Number.isFinite(value)) maxWidth.value = value
    }
  )
  ...
  watch(
    () => props.maxHeight,
    (value) => {
      if (value && Number.isFinite(value)) maxHeight.value = value
    }
  )
...
export function useObjectModel(props: PdfObjectProp) {
  const { objects, page: currentPage } = inject(PDF_OBJECTS_CONTEXT) || {}

  const page = ref(props.page ?? (currentPage?.value || 1))
  const x = ref(props.x)
  const y = ref(props.y)
  const id = Symbol('PDFObject')

  const { width, height, minWidth, maxWidth, minHeight, maxHeight, ratio } = useSizeModel(props)
